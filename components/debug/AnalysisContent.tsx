...
export const cleanMd = (text: string | undefined) => {
  if (!text) return "";
  return text.replace(/\\n(?![a-z])/g, "\n\n");
};
...
export const AnalysisContent: React.FC<ToolProps> = ({
  analysis,
  title,
  isPro,
  isResolving,
  ...
  return (
    <div
      className={`bg-white rounded-xl p-6 border shadow-sm ${isPro ? "border-purple-200 shadow-purple-50" : "border-slate-200"}`}
    >
      {/* Metadata Tags */}
      <div className="flex flex-wrap items-center justify-between gap-2 mb-4 pb-3 border-b border-slate-100">
        <div className="flex flex-wrap items-center gap-2">
          <span
            className={`text-[10px] font-black uppercase tracking-widest mr-2 flex items-center gap-1 ${isPro ? "text-purple-600" : "text-slate-500"}`}
          >
            {title || "AI 解析"}
          </span>
          ...
          <span
            className={`px-3 py-1 rounded text-xs font-bold border ${analysis.picture_ok ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200"}`}
          >
