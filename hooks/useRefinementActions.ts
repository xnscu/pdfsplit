...
export const useRefinementActions = ({ state, setters, actions, refreshHistoryList }: RefinementProps) => {
  ...
  const handleUpdateDetections = async (fileName: string, pageNumber: number, newDetections: DetectedQuestion[]) => {
    ...
    const oldPage = rawPages.find((p: DebugPageData) => p.fileName === fileName && p.pageNumber === pageNumber);
    ...
    try {
      const targetPages = updatedPages.filter((p: DebugPageData) => p.fileName === fileName);
      ...
      const existingFileQuestions = questions.filter((q: QuestionImage) => q.fileName === fileName) as QuestionImage[];
